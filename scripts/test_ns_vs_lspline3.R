...
rownames(meta) <- dge$samples$Sample.name[keep_final]
...
expr_mat <- matrix(as.numeric(edgeR::cpm(dge, log=TRUE)[1, keep_final]), nrow=1)
rownames(expr_mat) <- "gene1"
colnames(expr_mat) <- rownames(meta)
...
form_ls <- ~ (1|Subject.ID) + (1|RNA.isolation.Batch) + (1|Lib_prep_batches) + (1|Year.Drawn) + sex.numeric + RIN + mk_dup.PERCENT_DUPLICATION + star.uniquely_mapped_percent + Lymph.pcnt + Mid.pcnt + Gran.pcnt + MON.pcnt + EOS.pcnt + Age.months

vp_ns <- fitExtractVarPartModel(expr_mat, form_ns, meta, showWarnings=FALSE)
vp_ls <- fitExtractVarPartModel(expr_mat, form_ls, meta, showWarnings=FALSE)

print("vp_ns Subject.ID variance:")
print(vp_ns$Subject.ID)
print("vp_ls Subject.ID variance:")
