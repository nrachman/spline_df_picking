...
# Merge
meta <- dge_meta %>%
  inner_join(fardeep_df %>% select(Join_Key, all_of(cell_vars)), by = "Join_Key")

# Ensure unique and consistent indexing
rownames(meta) <- meta$DGE_Full_ID
dge_subset <- dge[, meta$DGE_Full_ID]

cat(sprintf("Using %d samples common to DGE and Deconvolution.\n", nrow(meta)))
...
continuous_vars <- c("RIN", "mk_dup.PERCENT_DUPLICATION", "star.uniquely_mapped_percent", "Age.months", cell_vars)
...
for(i in 1:nrow(expr_mat)) {
  ...
  tryCatch({
    ...
    vp_list[[i]] <- calc_vp_aggregated(m, age_terms, cell_vars)
...
saveRDS(vp_df, "output/rds/deconvolution_full_vp_results.rds")
cat("Results saved to output/rds/deconvolution_full_vp_results.rds\n")
